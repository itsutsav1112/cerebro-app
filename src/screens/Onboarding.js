...
export default function Onboarding({onComplete}) {
  ...
  const finish=()=>{
    ...
    const userData={why,mindset,tier,puzzleScores,persona,daysToCAT:days,attempts:[],sessionCount:0,streak:0,lastSeen:new Date().toISOString()};
    onComplete(userData);
  };
  ...
  if(step===1){
    ...
    return(
      <div style={S}>
        <Brand/><Progress pct={12}/><StepLabel n={1}/>
        ...
        <Btn label="Continue →" onClick={()=>setStep(2)} disabled={!why}/>
      </div>
    );
  }
  ...
  if(step===2){
    ...
    return(
      <div style={S}>
        <Brand/><Progress pct={28}/><StepLabel n={2}/>
        ...
        <Btn label="Continue →" onClick={()=>setStep(3)} disabled={mindset.filter(x=>x).length<3}/>
      </div>
    );
  }
  ...
  if(step===3){
    ...
    return(
      <div style={S}>
        <Brand/><Progress pct={44}/><StepLabel n={3}/>
        ...
        <Btn label="Continue →" onClick={()=>setStep(4)} disabled={tier===null}/>
      </div>
    );
  }
      ...
      <div style={S}>
        ...
        <div style={{background:'var(--surface)',border:'0.5px solid var(--surface3)',borderRadius:'var(--r)',padding:'18px',marginBottom:'14px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
            {p.opts.map((o,i)=>(
              <div key={i} onClick={()=>answerPuzzle(i)} style={{background:'var(--surface2)',border:'0.5px solid var(--hint)',borderRadius:'var(--r-sm)',padding:'10px 12px',cursor:'pointer',fontSize:'13px',color:'var(--text)',textAlign:'center'}}>{o}</div>
            ))}
          </div>
        </div>
      </div>
    ...
    return(
      <div style={{...S,paddingBottom:'2rem'}}>
        ...
        <div style={{background:'var(--gold-dim)',border:'0.5px solid var(--gold-border)',borderRadius:'var(--r)',padding:'14px',marginBottom:'1.25rem'}}>
          ...
          <div style={{fontSize:'13px',color:'var(--muted)',lineHeight:'1.65'}}>Most 99 percentilers don't attempt everything. They <strong style={{color:'var(--text)'}}>pick the right ones, stay calm, move fast.</strong></div>
        </div>
        ...
        <Btn label="Show me my profile →" onClick={()=>setStep(6)}/>
      </div>
    );
  }
  ...
  if(step===6){
    ...
    return(
      <div style={S}>
        ...
        <Btn label="Enter Cerebro →" onClick={finish}/>
      </div>
    );
  }
