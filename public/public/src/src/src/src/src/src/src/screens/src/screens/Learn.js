...
export default function Learn({ user }) {
  ...
  const loadQuestion = async (sec) => {
    ...
    try {
      ...
      setQuestion({
        type: 'Arithmetic',
        question: "A merchant marks goods 40% above cost and offers 20% discount. Selling price is ₹1,120. What is profit %?",
        sentences: [],
        options: [{l:'A',t:'10%'},{l:'B',t:'12%'},{l:'C',t:'11.2%'},{l:'D',t:'8%'}],
        correct: 'B',
        feedback_correct: "Correct. CP × 1.4 × 0.8 = 1.12×CP = 1120. Profit = 12%.",
        feedback_wrong: "CP × 1.4 × 0.8 = 1.12. So profit = 12%, not the multiplier itself.",
        shortcut: "Net %: a−b−(ab/100) = 40−20−8 = 12%"
      });
    }
    setLoading(false);
  };
  ...
  if (phase === 'intro') return (
    <div style={s}>
      ...
      <div style={{fontSize:'13px',color:'var(--muted)',lineHeight:'1.7',marginBottom:'1.5rem',padding:'14px',background:'var(--surface)',borderRadius:'var(--r)',border:'0.5px solid var(--surface3)'}}>
        {user?.persona?.coachNote || 'Your path is ready.'}
      </div>
      <div style={{fontSize:'14px',color:'var(--text)',lineHeight:'1.65',marginBottom:'1.5rem'}}>Before we dive into questions — would you like a quick breakdown of the exam? Format, strategy, what 99 actually requires?</div>
      <div style={{display:'flex',flexDirection:'column',gap:'10px',marginBottom:'1.5rem'}}>
        <div onClick={() => setWantsExamInfo(true)} style={{background:wantsExamInfo===true?'var(--gold-dim)':'var(--surface)',border:`${wantsExamInfo===true?'1px':'0.5px'} solid ${wantsExamInfo===true?'var(--gold)':'var(--hint)'}`,borderRadius:'var(--r)',padding:'14px 16px',cursor:'pointer',fontSize:'14px',color:'var(--text)'}}>
          Yes — walk me through the exam first
        </div>
        <div onClick={() => setWantsExamInfo(false)} style={{background:wantsExamInfo===false?'var(--gold-dim)':'var(--surface)',border:`${wantsExamInfo===false?'1px':'0.5px'} solid ${wantsExamInfo===false?'var(--gold)':'var(--hint)'}`,borderRadius:'var(--r)',padding:'14px 16px',cursor:'pointer',fontSize:'14px',color:'var(--text)'}}>
          Skip — let's get into questions
        </div>
      </div>
      {wantsExamInfo !== null && (
  ...
  if (phase === 'examinfo') return (
    <div style={{...s, paddingBottom:'2rem'}}>
      ...
      {[
        ...
        {title:'DILR strategy',content:'The first 5-6 minutes are sacred. Don\'t solve — scan. Go through all 4-5 sets, identify the 2 easiest to solve completely, and attack those first. Getting 3 sets fully correct puts you at 95-98 percentile in this section alone.'},
        {title:'QA strategy',content:'10th grade math, but fast. Most questions are from arithmetic, algebra, and geometry. The trick: if a question looks complicated, there\'s usually a shortcut. Cerebro will teach you those shortcuts, question by question.'},
      ...
      <div style={{background:'var(--gold-dim)',border:'0.5px solid var(--gold-border)',borderRadius:'var(--r)',padding:'14px 16px',marginBottom:'1.5rem'}}>
        <div style={{fontSize:'13px',color:'var(--text)',lineHeight:'1.65'}}>That's the map. Now let's figure out exactly where you are on it — and build the fastest path to 99.</div>
      </div>

      <button onClick={() => setPhase('sectionpick')} style={{background:'var(--gold)',color:'#0A0A0F',border:'none',borderRadius:'var(--r)',padding:'15px',fontFamily:'var(--serif)',fontSize:'15px',fontWeight:'700',cursor:'pointer',width:'100%'}}>
        Let's start →
      </button>
    </div>
  );
    ...
    <div style={s}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'10px',marginBottom:'1.5rem'}}>
        {Object.entries(SECTION_INFO).map(([sec, info]) => (
          <div key={sec} onClick={() => loadQuestion(sec)} style={{background:'var(--surface)',border:'0.5px solid var(--hint)',borderRadius:'var(--r)',padding:'16px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'space-between',transition:'all 0.15s'}}>
            <div>
      ...
      {qCount > 0 && (
        <div style={{background:'var(--surface)',border:'0.5px solid var(--surface3)',borderRadius:'var(--r)',padding:'12px 14px',display:'flex',justifyContent:'space-between'}}>
          <span style={{fontSize:'12px',color:'var(--muted)'}}>This session</span>
          <span style={{fontSize:'12px',color:'var(--gold)'}}>{correct}/{qCount} correct</span>
        </div>
      )}
    </div>
  );
        ...
        <>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'8px',marginBottom:'14px'}}>
            {question.options?.map((o) => (
              <div key={o.l} onClick={() => answer(o.l)} style={{
                background: answered && o.l === question.correct ? 'rgba(29,158,117,0.12)' : answered && o.l === selected && o.l !== question.correct ? 'rgba(226,75,74,0.12)' : selected === o.l ? 'var(--gold-dim)' : 'var(--surface)',
                border: `0.5px solid ${answered && o.l === question.correct ? '#1D9E75' : answered && o.l === selected && o.l !== question.correct ? '#E24B4A' : selected === o.l ? 'var(--gold)' : 'var(--hint)'}`,
                borderRadius:'var(--r-sm)',padding:'12px 14px',cursor:answered?'default':'pointer',
                display:'flex',alignItems:'center',gap:'10px',fontSize:'13.5px',color:'var(--text)',transition:'all 0.15s'
              }}>
