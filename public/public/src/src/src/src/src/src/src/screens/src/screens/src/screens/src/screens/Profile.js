  ...
  return (
    <div style={s}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '1.25rem' }}>
        {[
          { n: total, l: 'Questions answered' },
          { n: accuracy + '%', l: 'Overall accuracy' },
          { n: avgTime + 's', l: 'Avg time per question' },
          { n: user?.sessionCount || 0, l: 'Sessions completed' }
        ].map((item, i) => (
      ...
      {total === 0 && (
        <div style={{ fontSize: '13px', color: 'var(--muted)', textAlign: 'center', padding: '2rem 1rem', lineHeight: '1.6' }}>
          No questions answered yet. Head to Learn to get started.
        </div>
      )}
    </div>
  );
